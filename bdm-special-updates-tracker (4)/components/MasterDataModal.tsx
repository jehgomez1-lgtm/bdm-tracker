...
export const MasterDataModal: React.FC<MasterDataModalProps> = ({ currentMaster, onUpdate, onClose }) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          // Support both internal keys and spreadsheet header names
          // Spreadsheet: ID, City, Name
          // Internal: hhid, municipality, granteeName
          const idVal = row.hhid || row.id;
          const provVal = row.province;
          const muniVal = row.municipality || row.city;
          const brgyVal = row.barangay;
          const nameVal = row.granteename || row.grantee_name || row.name;

          if (idVal) {
            newRecords.push({
              hhid: idVal,
              province: provVal || 'MASBATE',
        ...
        setError("Error parsing master CSV. Ensure headers are present (ID, Province, City, Barangay, Name)");
      }
    };
  ...
  const downloadTemplate = () => {
    // Template matching the Google Sheet structure to be helpful
