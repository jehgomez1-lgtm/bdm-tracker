...
const StatusBadge: React.FC<{ status: UpdateStatus }> = ({ status }) => {
  ...
  return <span className={`px-2 py-0.5 rounded text-[10px] font-black border uppercase tracking-widest whitespace-nowrap ${activeStyle}`}>{status}</span>;
};
...
export const DetailTable: React.FC<DetailTableProps> = ({ 
  ...
  const renderCell = (rec: MemberRecord, key: keyof MemberRecord) => {
    switch (key) {
      case 'id':
        // Strip the unique suffix for display purposes (e.g. "HHID_uid_123" -> "HHID")
        const displayId = rec.id.split('_uid_')[0];
        return <td key={key} className="px-4 py-3 border-r font-mono font-black text-slate-700">{displayId}</td>;
      case 'province':
      ...
      case 'updateType':
        return <td key={key} className="px-4 py-3 border-r italic text-gray-500 text-[10px] uppercase tracking-tight">{rec.updateType}</td>;
      case 'status':
