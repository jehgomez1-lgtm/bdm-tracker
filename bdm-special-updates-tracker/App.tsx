...
import { getMasterRecords, saveMasterRecords, countHouseholdRecords, openDB, clearHouseholdRecords } from './utils/db';
...
const App: React.FC = () => {
  ...
  const [filterStatus2, setFilterStatus2] = useState<string | null>(null);
  ...
  const handleSaveUrl = (url: string) => {
    setCloudUrl(url);
    localStorage.setItem('bdm_cloud_url', url);
  };
  ...
  const handleGuestLogin = () => {
    ...
    setIsTerminated(false);
    setIsDeleted(false);
    logAction('Guest', 'SESSION_START', 'Guest accessed the dashboard');
  };
  ...
  const handleClearProfiles = async () => {
    if (!currentUser) return;
    if (window.confirm("WARNING: You are about to delete ALL local household profile data.\n\nThis is typically done before uploading a fresh dataset to remove obsolete records.\n\nAre you sure you want to proceed?")) {
        try {
            await clearHouseholdRecords();
            setProfileCount(0);
            logAction(currentUser.name, 'HOUSEHOLD_CLEAR', 'Purged local household database');
            alert("Database cleared successfully. You can now upload the new CSV file.");
        } catch (e) {
            console.error(e);
      ...
      if (Array.isArray(rawRecords)) {
        const normalized = rawRecords.map((item: any) => {
           ...
           return {
             ...
             // Corrected Mapping based on Sheet Structure: Status1(9), Date1(10), Status2(11), Date2(12)
             statusDate1: String(isArr ? item[10] : item.statusDate1 || ''), // Col K
             status2: (isArr ? item[11] : item.status2 || '') as UpdateStatus, // Col L
             statusDate2: String(isArr ? item[12] : item.statusDate2 || ''), // Col M
             extraInfo: String(isArr ? item[13] : item.extraInfo || '')      // Col N
           };
  ...
  const summaryData = useMemo(() => {
    const dataMap = new Map<string, MemberRecord[]>();

    // Group all records by their exact updateType string
    // This catches everything: standard types, misspelled types, and empty types
    records.forEach(r => {
      // Handle null/undefined/empty string by grouping them into "(Unspecified)"
      const key = (r.updateType && r.updateType.trim() !== '') ? r.updateType : '(Unspecified)';
      if (!dataMap.has(key)) {
        dataMap.set(key, []);
      }
      dataMap.get(key)!.push(r);
    });

    // Create rows from the map
    const rows: { updateType: string, periods: number[], total: number }[] = [];
    
    dataMap.forEach((recs, type) => {
       const periods = [1, 2, 3, 4, 5, 6].map(p => 
         recs.filter(r => r.period === p).length
       );
       rows.push({
         updateType: type,
         periods,
    ...
    // Sorting Logic: 
    // 1. Standard UPDATE_TYPES come first, in the order defined in constants.ts
    // 2. Non-standard types follow, sorted by Total Volume (Descending)
    rows.sort((a, b) => {
        const idxA = UPDATE_TYPES.indexOf(a.updateType);
        const idxB = UPDATE_TYPES.indexOf(b.updateType);

        // If both are standard types, sort by index in UPDATE_TYPES
        ...
        // If neither are standard, sort by total count desc
  ...
  const filteredRecords = useMemo(() => {
    let result = records.filter(r => {
      ...
      const matchStatus2 = !filterStatus2 || r.status2 === filterStatus2;
      return matchSearch && matchMuni && matchType && matchPeriod && matchStatus2;
    });
    ...
    return result;
  }, [records, searchQuery, filterMuni, filterType, filterPeriod, filterStatus2, sortOption]);
        ...
        {showProfile ? (
          ...
          <>
            ...
            {activeTab === 'details' && (
               <div className="space-y-6">
                 ...
                 <DetailTable 
                    ...
                    onFilterStatus2Change={setFilterStatus2}
            ...
            {activeTab === 'profiles' && (
              <HouseholdView 
                records={records}
                onImportClick={isGuest ? undefined : () => setShowHouseholdModal(true)} 
                onSyncCloud={isGuest ? undefined : syncProfilesToCloud}
                onClearDatabase={isGuest ? undefined : handleClearProfiles}
                isSyncing={isSyncing}
              />
            )}
            
            {activeTab === 'setup' && (
              <SetupGuide currentUrl={cloudUrl} onSaveUrl={handleSaveUrl} onRetry={syncFromCloud} onManageMaster={() => setShowMasterModal(true)} />
            )}
          </>
        )}
      </main>
