...
export const DataEntryForm: React.FC<DataEntryFormProps> = ({ 
  ...
  const [formData, setFormData] = useState({
    ...
    statusDate1: new Date().toISOString().split('T')[0],
    status2: '' as UpdateStatus | '',
    statusDate2: '',
  ...
  const masterCount = masterList.length;
  ...
  useEffect(() => {
    if (initialRecord) {
      ...
      // Extract MID if present in extraInfo (format: "... (MID: xxxxx)")
      let initialMid = '';
      let extraInfo = initialRecord.extraInfo || '';
      const midMatch = extraInfo.match(/\(MID: (.*?)\)/);
      if (midMatch) {
        initialMid = midMatch[1];
        // Clean extra info for display if needed, but keeping it raw is safer for now to avoid data loss on resave
        // extraInfo = extraInfo.replace(midMatch[0], '').trim(); 
      }
      ...
      setFormData({
        ...
        statusDate1: initialRecord.statusDate1 || initialRecord.date,
        status2: initialRecord.status2 || '',
        statusDate2: initialRecord.statusDate2 || '',
        attending: 'Yes',
        reasonResolution: extraInfo, // Keeping raw extraInfo
      ...
      setMembers([{ name: initialRecord.memberName, mid: initialMid }]);
      setIsVerified(true);
    }
  }, [initialRecord]);
  ...
  const isUpdate11 = formData.updateType.includes("UPDATE 11");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation for MID removed as per user request (it is now optional)
    ...
    for (const m of members) {
      ...
      const isDuplicate = existingRecords.some(r => {
        ...
        return (rHhid === hhidToCheck && rType === typeToCheck && rName === memberNameToCheck);
      });
    ...
    try {
        for (const m of members) {
            ...
            // Append MID to details if present and required
            if (needsMID && m.mid && m.mid.trim() !== "") {
                // Check if MID is already in details (edit mode) to avoid duplication
                if (!details.includes(`(MID: ${m.mid})`)) {
                   details = `${details} (MID: ${m.mid})`.trim();
                }
            }
            ...
            const recordToSave: MemberRecord = {
                ...
                statusDate1: formData.statusDate1,
                status2: formData.status2 || undefined,
                statusDate2: formData.statusDate2,
                extraInfo: details
            };
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center mt-2 space-x-3">
              ...
              <p className="text-[10px] font-bold text-slate-300">
                DB: {masterCount.toLocaleString()} RECORDS
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-10 max-h-[85vh] overflow-y-auto custom-scrollbar">
          ...
          <section className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              {masterCount === 0 && !isManualMode && (
                  <p className="text-[9px] text-red-400 font-bold ml-2">
                      <i className="fa-solid fa-triangle-exclamation mr-1"></i>
                      Local Master Database is empty. Go to <strong>Setup &gt; Manage Master Data</strong> to sync.
                  </p>
              )}
            </div>

            {/* Simplified Location & Grantee for brevity in this response (keeping original styles) */}
                 ...
                 <div className="grid grid-cols-2 gap-x-8 gap-y-6">
                    ...
                    <div className="col-span-2 space-y-1 pt-4 border-t border-slate-50">
                      ...
                      <input 
                         ...
                         className="w-full bg-transparent font-black text-[#1e293b] text-[15px] truncate uppercase tracking-tighter leading-none focus:border-indigo-500 outline-none border-b border-transparent focus:border-indigo-200 placeholder:text-slate-300"
                         placeholder={isManualMode ? "ENTER NAME..." : "AUTO-FILLS FROM MASTER"}
                      />
                    </div>
                 </div>
               </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {needsMID && (
                       <div className="space-y-1 animate-in fade-in slide-in-from-top-2">
                          <label className="text-[9px] font-black text-indigo-400 uppercase tracking-widest ml-1">Member ID (Optional)</label>
                          <input 
                            type="text" 
                            placeholder="Enter Member ID..." 
                            value={m.mid}
                            onChange={(e) => updateMember(i, 'mid', e.target.value)}
                            className="w-full bg-transparent border-b-2 border-indigo-100 py-3 text-sm font-black outline-none focus:border-indigo-500 transition-colors placeholder:text-indigo-200 text-indigo-900"
                          />
                       </div>
                    )}
                  </div>
          ...
          <section className="space-y-6">
            <div className="flex items-center space-x-3">
              <div className="w-1.5 h-6 bg-[#1e293b] rounded-full"></div>
              <h3 className="text-[13px] font-black text-[#1e293b] uppercase tracking-widest">3. Classification & Status</h3>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-100">
                 ...
                 <div className="space-y-3">
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-orange-500 uppercase tracking-widest ml-1 flex items-center">
                           <i className="fa-solid fa-pen-to-square mr-1"></i> Update Status 1
                        </label>
                        <div className="relative">
                           <select 
                              value={formData.docStatus} 
                              onChange={e => setFormData({...formData, docStatus: e.target.value as UpdateStatus})} 
                              className="w-full pl-4 pr-8 py-3 bg-orange-50/50 border border-orange-200 rounded-2xl text-[11px] font-bold text-orange-800 outline-none appearance-none cursor-pointer focus:border-orange-500 focus:ring-2 focus:ring-orange-500/10 transition-all shadow-sm"
                           >
                              {STATUS_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
                           </select>
                           <i className="fa-solid fa-chevron-down absolute right-4 top-1/2 -translate-y-1/2 text-orange-400 text-[10px] pointer-events-none"></i>
                        </div>
                    </div>
                    <div className="space-y-1">
                        <label className="text-[9px] font-black text-slate-400 uppercase ml-1">Date 1</label>
                        <input 
                            type="date"
                            value={formData.statusDate1}
                            onChange={e => setFormData({...formData, statusDate1: e.target.value})}
                            className="w-full px-4 py-3 bg-white border border-slate-200 rounded-2xl text-[11px] font-bold text-slate-600 outline-none focus:border-orange-500 transition-all" 
                        />
                    </div>
                 </div>
                 ...
                 <div className="space-y-3">
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-indigo-500 uppercase tracking-widest ml-1 flex items-center">
                           <i className="fa-solid fa-check-double mr-1"></i> Update Status 2
                        </label>
                        <div className="relative">
                           <select 
                              value={formData.status2} 
                              onChange={e => setFormData({...formData, status2: e.target.value as UpdateStatus})} 
                              className="w-full pl-4 pr-8 py-3 bg-indigo-50/50 border border-indigo-200 rounded-2xl text-[11px] font-bold text-indigo-800 outline-none appearance-none cursor-pointer focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10 transition-all shadow-sm"
                           >
                    ...
                    <div className="space-y-1">
                        <label className="text-[9px] font-black text-slate-400 uppercase ml-1">Date 2</label>
                        <input 
                            type="date"
                            value={formData.statusDate2}
                            onChange={e => setFormData({...formData, statusDate2: e.target.value})}
                            className="w-full px-4 py-3 bg-white border border-slate-200 rounded-2xl text-[11px] font-bold text-slate-600 outline-none focus:border-indigo-500 transition-all" 
                        />
                    </div>
                 </div>
              </div>
              ...
              {(showReasonDiv || showAttending) && (
                <div className="bg-indigo-50/40 p-10 rounded-[2.5rem] border-2 border-indigo-50 space-y-8 mt-6">
                  {showAttending && (
                  ...
                  {(!showAttending || formData.attending === 'No') && showReasonDiv && (
                    <div className="space-y-4">
                      <label className="text-[11px] font-black text-indigo-900 uppercase tracking-widest block">
                        {showAttending ? 'Details / Reason' : isUpdate11 ? 'Selection Action' : 'Resolution / Level'}
                      </label>
                      <select 
