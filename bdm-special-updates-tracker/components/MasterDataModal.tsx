...
export const MasterDataModal: React.FC<MasterDataModalProps> = ({ cloudUrl, currentMaster, onUpdate, onClose }) => {
  ...
  const parseAndSave = (rows: any[], headers: string[]) => {
    ...
    // Helper to find key in row object regardless of case or spacing
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        // Normalize keys with robust fallback options
        const idVal = getValue(row, ['hhid', 'id', 'hh id', 'hh_id', 'hh-id']);
        const provVal = getValue(row, ['province', 'prov']);
        const muniVal = getValue(row, ['municipality', 'city', 'muni']);
        const brgyVal = getValue(row, ['barangay', 'brgy']);
        const nameVal = getValue(row, ['granteename', 'grantee_name', 'name', 'grantee name', 'grantee']);
  ...
  const handleCsvUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const rows = lines.map(line => line.split(/,(?=(?:(?:[^"]*"){2})*[^"]*$)/).map(v => v.trim().replace(/["']/g, "")));
  ...
  const downloadTemplate = () => {
    const headers = "HH_ID,Client Status,City,Barangay,Grantee Name,Province";
