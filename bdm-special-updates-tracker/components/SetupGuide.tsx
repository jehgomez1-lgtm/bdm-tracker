...
export const SetupGuide: React.FC<SetupGuideProps> = ({ currentUrl, onSaveUrl, onRetry, onWipe, onManageMaster }) => {
  ...
  // Auto-verify connection if a URL is already present
  useEffect(() => {
    if (currentUrl) {
  ...
  const testConnection = async (urlOverride?: string) => {
    const urlToTest = urlOverride || urlInput;
    if (!urlToTest) return;
    
    setTestStatus('testing');
    try {
      // Small artificial delay only if it's a manual test to show the spinner
      if (!urlOverride) await new Promise(resolve => setTimeout(resolve, 1500));
      
      // Test the link with a sheet parameter to verify script compatibility
      const urlWithParam = urlToTest.includes('?') ? `${urlToTest}&sheet=HHID+STATUS` : `${urlToTest}?sheet=HHID+STATUS`;
      ...
      if (isValid) {
        ...
        if (!urlOverride) onSaveUrl(urlToTest); // Only auto-save if manual test
      } else {
      ...
      <div className="bg-[#020617] rounded-[3rem] p-10 md:p-16 text-white shadow-2xl relative overflow-hidden border border-white/10">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
          ...
          <div className="bg-white/5 backdrop-blur-xl p-10 rounded-[3rem] border border-white/10 text-center flex flex-col items-center">
             <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-4 shadow-xl transition-all ${testStatus === 'success' ? 'bg-green-500 shadow-green-500/20' : 'bg-indigo-600 shadow-indigo-500/20'}`}>
                <i className={`fa-solid ${testStatus === 'success' ? 'fa-link' : 'fa-satellite-dish'} text-white text-2xl`}></i>
             </div>
             <div className={`text-[10px] font-black uppercase tracking-widest ${testStatus === 'success' ? 'text-green-400' : 'text-indigo-400'}`}>
                {testStatus === 'success' ? 'Bridge Active' : 'Ready to Link'}
             </div>
          </div>
        </div>
      </div>
              ...
              <div className="space-y-6">
                 ...
                 <button 
                   ...
                   className={`w-full py-6 rounded-3xl text-[12px] font-black uppercase tracking-widest transition-all shadow-xl flex items-center justify-center space-x-4 ${
                     testStatus === 'success' ? 'bg-green-600 text-white shadow-green-500/20' : 
