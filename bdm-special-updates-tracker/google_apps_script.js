...
function saveProfiles(records, mode) {
  ...
  // Create a single comma-separated header string
  const headerString = HEADERS_PROFILE.join(',');

  if (!sheet) {
    sheet = ss.insertSheet(SHEET_PROFILE_NAME);
    // Write header in cell A1
    sheet.getRange(1, 1).setValue(headerString);
  }
  ...
  if (mode === 'replace') {
    ...
    // Clear previous data
    if (lastRow > 1) {
       // Clear broadly to handle transitions from multi-column to single-column
       sheet.getRange(2, 1, lastRow - 1, lastCol || 20).clearContent();
    }
    
    // Check if header needs refresh (e.g. converting from multi-col to single-col)
    // If B1 has content, it implies the headers are spread out in the old format.
    const checkHeader = sheet.getRange(1, 2).getValue();
    if (checkHeader !== "") {
       sheet.clear();
       sheet.getRange(1, 1).setValue(headerString);
    }
  ...
  const rows = records.map(r => {
    ...
    // Sanitize and join with commas
    // Replaces internal commas with spaces to prevent CSV parsing issues if exported later
